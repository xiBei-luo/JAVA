...
function initData(data){
    for (var i=0; i<data.length; i++){
        ...
        if ("1" == data[i].cRylb){
            opLinkBuff.append("<button type=\"button\" class=\"btn btn-sm btn-primary \" onclick=\"f_upd(\'"+data[i].cUserid+"\');\"  >修改</button>&nbsp&nbsp&nbsp");
            opLinkBuff.append("<button type=\"button\" class=\"btn btn-sm btn-danger \" onclick=\"f_del(\'"+data[i].cUserid+"\');\"  >删除</button>&nbsp&nbsp&nbsp");
            opLinkBuff.append("<button type=\"button\" class=\"btn btn-sm btn-warning \" onclick=\"f_retPass(\'"+data[i].cUserid+"\');\"  >重置密码</button>");
